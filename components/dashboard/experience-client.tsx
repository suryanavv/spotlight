...
import { createExperience, updateExperience, deleteExperience } from '@/supabase/client-actions'
...
export default function ExperienceClient() {
  ...
  // Get preloaded experience data from context
  const experienceList = dashboardData?.experience || []
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingExperience) {
        const result = await updateExperience(editingExperience.id, experienceData)
        if (result.error) {
          toast.error('Failed to update experience')
          console.error('Error updating experience:', result.error)
          return
        } else {
        ...
        if (result.error) {
          ...
          console.error('Error creating experience:', result.error)
          return
        } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        console.error('Error deleting experience:', result.error)
      } else {
