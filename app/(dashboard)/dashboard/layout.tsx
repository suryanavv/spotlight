...
const DashboardMobileMenu = ({ mobileMenuOpen, setMobileMenuOpen }: { mobileMenuOpen: boolean; setMobileMenuOpen: (open: boolean) => void }) => {
  const { user, signOut } = useAuth()
  const router = useRouter()
  const pathname = usePathname()
  const queryClient = useQueryClient()
...
const DashboardContent = ({ children, sidebarOpen }: { children: React.ReactNode; sidebarOpen: boolean }) => {
  const pathname = usePathname()
