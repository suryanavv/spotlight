...
import { useBlogs, useBlogMutations } from '@/lib/hooks/useQueries';
...
export default function BlogsPage() {
  ...
  const { data: blogs = [], isInitialLoading, hasData, error } = useBlogs();
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <p className="text-muted-foreground">Failed to load blogs data</p>
          <Button
            variant="outline"
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteBlog.mutateAsync(deletingId)
  ...
  const renderForm = () => (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        >
          {createBlog.isPending || updateBlog.isPending ? "Saving..." : editingBlog ? "Update" : "Add"} Blog Post
        </Button>
      </div>
    </form>
  )
